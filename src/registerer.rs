...
pub async fn run_elevated(command: &str, app_id: String, display_name: Option<String>, icon_uri: Option<String>) -> Result<(), String> {
...
pub fn register_app_id(app_id: String, display_name: Option<String>, icon_path: Option<String>) -> Result<(), RegistrationError> {
    if app_id.contains(r"\") || app_id.contains("/") {
        return Err(RegistrationError::ArgumentError(format!("app id [{}] contains invalid characters", app_id)));
    }
    if let Some(icon) = &icon_path {
        ...
        match path.extension().ok_or(RegistrationError::ArgumentError(extension_error.clone()))? {
            ext => {
                if !ext.eq("png") {
    ...
    update_value(&app_id_sub_key, "DisplayName", display_name.or(Some(app_id.to_string())))?;
